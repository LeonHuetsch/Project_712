function vEqConditions = ConditionsGE(nGridAsset,minAsset,...
    maxAsset,nGridShock,ssigmaY,ddelta,logShockAverage,truncOpt,...
    rrho,aalpha,A,depreciation,ssigma,mValueGuess,llambda,r,kkappa,ttau)
...
[vGridAsset,vGridShock,mTransitionShock] = SetupGrids(...
    nGridAsset,minAsset,maxAsset,nGridShock,ssigmaY,ddelta,...
    logShockAverage,truncOpt);

[~,~,~,mIndexPolicyAsset,mPolicyLabor] = ...
    VFiteration_UBI(ttau,llambda,kkappa,rrho,r,aalpha,A,depreciation,ssigma,vGridAsset,...
    vGridShock,mTransitionShock,mValueGuess);

[mStationaryDist,expectAssetHoldings] = StationaryDist...
    (vGridAsset,nGridShock,mTransitionShock,mIndexPolicyAsset);
